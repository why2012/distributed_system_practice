...
import org.elasticsearch.primaryaware.action.support.replication.TransportAsyncReplicationAction;
import org.elasticsearch.primaryaware.action.support.replication.TransportAsyncWriteAction;
...
public class TransportAsyncShardBulkAction extends TransportAsyncWriteAction<BulkShardRequest, BulkShardRequest, BulkShardResponse> {

    public static final String ACTION_NAME = AsyncBulkAction.NAME + "[s]";
    ...
    @Inject
    public TransportAsyncShardBulkAction(Settings settings, TransportService transportService, ClusterService clusterService,
    ...
    protected TransportRequestOptions transportOptions(Settings settings) {
        return AsyncBulkAction.INSTANCE.transportOptions(settings);
    }

    @Override
    protected BulkShardResponse newResponseInstance() {
        return ReflectionUtils.newInstance(BulkShardResponse.class, new Class[]{}, new Object[]{});
    }
    ...
    protected void shardOperationOnPrimary(BulkShardRequest request, IndexShard primary,
                                           ActionListener<TransportAsyncReplicationAction.PrimaryResult<BulkShardRequest, BulkShardResponse>> listener) {
    ...
    public static void performOnPrimary(
        ...
        new ActionRunnable<TransportAsyncReplicationAction.PrimaryResult<BulkShardRequest, BulkShardResponse>>(listener) {
            ...
            private void finishRequest() {
                ActionListener.completeWith(listener,
                        () -> new TransportAsyncWriteAction.WritePrimaryResult<>(
    ...
    private static void onComplete(Engine.Result r, BulkPrimaryExecutionContext context, UpdateHelper.Result updateResult) {
        ...
        if (isUpdate) {
            ...
            if (isFailed) {
                ...
                if (TransportAsyncShardBulkAction.isConflictException(failure)) {
     ...
     *
     * @return {@link TransportShardBulkAction.ReplicaItemExecutionMode#NORMAL} upon normal primary execution with no failures
     * {@link TransportShardBulkAction.ReplicaItemExecutionMode#FAILURE} upon primary execution failure after sequence no generation
     * {@link TransportShardBulkAction.ReplicaItemExecutionMode#NOOP} upon primary execution failure before sequence no generation or
     * when primary execution resulted in noop (only possible for write requests from pre-6.0 nodes)
     */
    static TransportShardBulkAction.ReplicaItemExecutionMode replicaItemExecutionMode(final BulkItemRequest request, final int index) {
        final BulkItemResponse primaryResponse = ReflectionUtils.invokeMethod(request, "getPrimaryResponse", new Class[]{}, new Object[]{});
        ...
        if (primaryResponse.isFailed()) {
            return primaryResponse.getFailure().getSeqNo() != SequenceNumbers.UNASSIGNED_SEQ_NO
                    ? TransportShardBulkAction.ReplicaItemExecutionMode.FAILURE // we have a seq no generated with the failure, replicate as no-op
                    : TransportShardBulkAction.ReplicaItemExecutionMode.NOOP; // no seq no generated, ignore replication
        } else {
            ...
            return primaryResponse.getResponse().getResult() != DocWriteResponse.Result.NOOP
                    ? TransportShardBulkAction.ReplicaItemExecutionMode.NORMAL // execution successful on primary
                    : TransportShardBulkAction.ReplicaItemExecutionMode.NOOP; // ignore replication
        }
    }

    @Override
    public TransportAsyncWriteAction.WriteReplicaResult<BulkShardRequest> shardOperationOnReplica(BulkShardRequest request, IndexShard replica) throws Exception {
        final Translog.Location location = performOnReplica(request, replica);
        return new TransportAsyncWriteAction.WriteReplicaResult<>(request, location, null, replica, logger);
    }
    ...
    public static Translog.Location performOnReplica(BulkShardRequest request, IndexShard replica) throws Exception {
        ...
        for (int i = 0; i < request.items().length; i++) {
            ...
            BulkItemResponse bulkItemResponse = ReflectionUtils.invokeMethod(item, "getPrimaryResponse", new Class[]{}, new Object[]{});
            final Engine.Result operationResult;
            DocWriteRequest<?> docWriteRequest = item.request();
            switch (replicaItemExecutionMode(item, i)) {
                case NORMAL:
                    final DocWriteResponse primaryResponse = bulkItemResponse.getResponse();
